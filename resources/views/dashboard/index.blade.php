        ...
        <div class="col-md-6">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    @if($prof->etudiants_encadres_count > 0)
                    <p style="margin-bottom: 4px; font-size: 13px;">
                        {{ $prof->nom }} {{ $prof->prenom }}
                        <span class="pull-right"><strong>{{ $prof->etudiants_encadres_count }}</strong></span>
                    </p>
                    <div class="barre-conteneur">
                        @php
                            $pourcent = $maxEtudiants > 0 ? ($prof->etudiants_encadres_count / $maxEtudiants) * 100 : 0;
                            $couleur = $prof->etudiants_encadres_count > 4 ? 'rouge' : ($prof->etudiants_encadres_count < 3 ? 'orange' : '');
                        @endphp
                        <div class="barre {{ $couleur }}" style="width: {{ $pourcent }}%">
                            {{ $prof->etudiants_encadres_count }}
                        </div>
                    </div>
                    @endif
                    @endforeach
        ...
        <div class="col-md-6">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    @if($prof->plannings_count > 0)
                    <p style="margin-bottom: 4px; font-size: 13px;">
                        {{ $prof->nom }} {{ $prof->prenom }}
                        <span class="pull-right"><strong>{{ $prof->plannings_count }}</strong></span>
                    </p>
                    <div class="barre-conteneur">
                        @php
                            $pourcent = $maxSoutenances > 0 ? ($prof->plannings_count / $maxSoutenances) * 100 : 0;
                        @endphp
                        <div class="barre" style="width: {{ $pourcent }}%">
                            {{ $prof->plannings_count }}
                        </div>
                    </div>
                    @endif
                    @endforeach
