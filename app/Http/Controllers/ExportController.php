...
class ExportController extends Controller
{
    ...
    public function download()
    {
        ...
        // En-têtes
        $headers = ['Jour', 'Heure Début', 'Heure Fin', 'Salle', 'Étudiant', 'Filière', 'Encadrant', 'Jury 2', 'Jury 3'];
        ...
        foreach ($plannings as $p) {
            $sheet->setCellValue("A{$row}", $p->creneau->jour ?? '-');
        ...
        // ── Feuille 2 : Par Jour ──
        $jours = $plannings->pluck('creneau.jour')->unique()->filter()->sort();
        foreach ($jours as $jour) {
            ...
            $jourSheet->getStyle('A1:I1')->applyFromArray($headerStyle);
            $jourSheet->getRowDimension(1)->setRowHeight(30);

            $jourPlannings = $plannings->filter(function ($p) use ($jour) {
                return ($p->creneau->jour ?? '') === $jour;
            });

            $row = 2;
            foreach ($jourPlannings as $p) {
                $jourSheet->setCellValue("A{$row}", $p->creneau->jour ?? '-');
                ...
                if ($row % 2 === 0) {
                    $jourSheet->getStyle("A{$row}:I{$row}")->getFill()
                        ->setFillType(Fill::FILL_SOLID)
                        ->getStartColor()->setRGB('EBF5FB');
                }
                $row++;
            ...
            if ($row > 2) {
                $jourSheet->getStyle('A1:I' . ($row - 1))->getBorders()->getAllBorders()
                    ->setBorderStyle(Border::BORDER_THIN)
                    ->getColor()->setRGB('CCCCCC');
            }
